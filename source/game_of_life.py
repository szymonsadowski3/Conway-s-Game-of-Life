...
class GameOfLifeGrid(Gtk.Window):
    ...
    def colorSwatch(self, str_color):
        return helpers.GtkHelper.generateCell(str_color, self.cell_size)

    def generateGridOfCells(self, grid, init_color=helpers.ConfigLoader.getDefaultValueFor('DEFAULT_BG_COLOR')):
    ...
    def changeCellColor(self, row, column, color):
        ...
        helpers.GtkHelper.overrideCellColor(cell, color)

    def changeCellColorToBG(self, row, column):
        is_cell_odd = (row+column)%2==0
        cell = self.grid.get_child_at(column, row)
        if is_cell_odd:
            helpers.GtkHelper.overrideCellColor(cell, helpers.ConfigLoader.getDefaultValueFor('DEFAULT_BG_COLOR'))
        else:
            helpers.GtkHelper.overrideCellColor(cell, helpers.ConfigLoader.getDefaultValueFor('DEFAULT_BG_COLOR2'))

    def changeCellColorToForeground(self, row, column):
        cell = self.grid.get_child_at(column, row)
        helpers.GtkHelper.overrideCellColor(cell, helpers.ConfigLoader.getDefaultValueFor('DEFAULT_FOREGROUND_COLOR'))

    def updateVisualization(self):
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                if self.model[i][j]!=self.model_buffer and self.model[i][j]:
                    self.changeCellColor(i,j, helpers.ConfigLoader.getDefaultValueFor('DEFAULT_FOREGROUND_COLOR'))
    ...
    def __init__(self):
        ...
        save_button = Gtk.Button("Restart Program")
        save_button.connect("clicked", helpers.restartProgram)
        hb.add(save_button)
        #CONFIG DIALOG
